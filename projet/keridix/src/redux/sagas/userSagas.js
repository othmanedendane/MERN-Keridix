...
function signUpApi(username, password) {
	return axios({
		...
		withCredentials: true,
		url: "http://localhost:3510/register",
	})
		.then(res => {
...
function getCurrentUser() {
	return axios({
		method: "GET",
		withCredentials: true,
		url: "http://localhost:3510/account"
		})
		.then(res => {
			return res.data
		})
...
function changePasswordApi(user, oldPassword, newPassword) {
	return axios({
			...
			withCredentials: true,
			url: "http://localhost:3510/change-password",
		})
		.then(res => {
			return res.data
		})
...
function signOutApi() {
	return 	axios({
				method: "GET",
				withCredentials: true,
				url: "http://localhost:3510/logout"
			})
			.then(res => {
				console.log(res);
				return res.data
